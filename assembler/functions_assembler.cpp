...
std::array<uint16_t, 4> convert(std::string line) {
    ...
    */
    ...
    if (addrmodeencoding.count(line[3]) > 0) {
        ...
        std::cerr << "Instruction \"" << line << "\" cannot be converted: bad addressing mode\n";
        std::exit(-1);
    }
    if (registerencoding.count(line[4]) > 0) {
        data[1] |= registerencoding.at(line[4]);
    } else {
        std::cerr << "Instruction \"" << line << "\" cannot be converted: bad left hand register\n";
        std::exit(-1);
    }
    ...
    if (registerencoding.count(line[5]) > 0) {
        ...
        std::cerr << "Instruction \"" << line << "\" cannot be converted: bad right hand register\n";
        std::exit(-1);
    }
